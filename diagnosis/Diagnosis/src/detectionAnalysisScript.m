...
DO_FP_DISPLAY = 0;   % display FP analysis
...
% detectors = {'DetectoRS',  'Foveabox', 'GridRCNN', 'GuidedAnchor', 'CascadeRCNN', 'Retinanet',  'RepPoints'};  % detectors that will be analyzed
detectors = {'ATSS'};
...
for d = 1:numel(detectors)  % loops through each detector and performs analysis
  ...
  if DO_TP_DISPLAY
    ...
    if ~exist(outfile, 'file') || ~SKIP_SAVED_FILES
      detail_subset = [1]; % objects for which to create per-object detailed plots
      plotnames = {'occlusion', 'area', 'height', 'aspect', 'truncation', 'parts', 'view'};
      
      clear result;
      for o = 1:numel(objnames_selected)
        tmp = load(fullfile(resultdir, sprintf('results_%s_%s.mat', objnames_selected{o}, localization)));
        result(o) = tmp.result;
      end
...
%       for f = 1:numel(objnames_selected)
%         set(f, 'PaperUnits', 'inches'); 
%         set(f, 'PaperSize', [8.5 11]); 
%         set(f, 'PaperPosition', [0 11-3 8 2.5]);
%         print('-dpdf', ['-f' num2str(f)], fullfile(resultdir, sprintf('plots_%s_%s.pdf', objnames_selected{f}, localization)));
%       end
